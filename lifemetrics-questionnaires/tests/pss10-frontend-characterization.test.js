...
const restControllerPath = path.join(testsDir, '..', 'includes', 'class-rest-controller.php');
...
function runCharacterization(mutationName) {
  ...
  const restController = fs.readFileSync(restControllerPath, 'utf8');
  ...
  assert.ok(restController.includes("'/pss10/submit'"));
  ...
  assert.ok(template.includes('data-lmq-role="gauge-track"'));
  assert.ok(template.includes('data-lmq-role="gauge-marker"'));
  assert.ok(template.includes('data-lmq-role="gauge-gradient"'));
  assert.ok(!template.includes('gauge-needle'));
  assert.ok(!template.includes('gauge-fill'));
  assert.ok(template.includes('data-lmq-role="analysis-lead"'));
  assert.ok(template.includes('data-lmq-role="analysis-details"'));
  assert.ok(source.includes('renderGaugeGradient();'));
  assert.ok(source.includes('renderGaugeMarker(score, category);'));
  assert.ok(!source.includes('gaugeNeedle'));
  assert.ok(!source.includes('gaugeFill'));
  ...
  assert.ok(css.includes('.lmq-pss10 .gauge-track'));
  assert.ok(css.includes('.lmq-pss10 .gauge-marker'));
  ...
  assert.match(css, /\.lmq-pss10 \.analysis-toggle:hover[\s\S]*?background: transparent !important;/);
