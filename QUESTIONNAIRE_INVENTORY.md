...
| `nutrition` | `CONTENT_READY` | model/ranges defined; completed synthetic validation section absent | NTSF01-NTSF03 | none | none | VitaScan | `BLOCKED_BY_SCORING` | scoring profiles/boundaries plus owner/date/URL pending |
| `fatigue-recuperation` | `CONTENT_READY` | `SCORING_READY` based on documented synthetic validation | FRSF01-FRSF03 | none | weakest 1-2; attention <=2/8 | VitaScan + Metabolism Analytics | `BLOCKED_BY_APPROVAL` | source owner/date and final CTA destinations pending |
| `sedentarite` | `CONTENT_READY` | `SCORING_READY`; documented profiles A-F | none | SD07, SD08 | D1 <=2/8 caps display at `SEDENTARITE_A_REDUIRE`; weakest/attention | VitaScan + Metabolism Analytics | `BLOCKED_BY_APPROVAL` | explicit source/stage approval and CTA destinations pending |
